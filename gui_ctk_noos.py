...
class App(customtkinter.CTk):
    ...
    def get_image_path(self):
        self.image_path = filedialog.askopenfilename(initialdir=Path().resolve(), title="Select an image file",  
    ...
    def single_classification(self):
        if self.is_running:
            ...
            model_path = str(Path("./model/"+ model_name).resolve())
    ...
    def batch_classification(self):
        if self.is_running:
            ...
            # Get the file path of the model
            model_name = "model.tflite"
            model_path = Path("./model/"+ model_name).resolve()
            # model_path = Path(model_name).resolve()
            # print(model_path)
